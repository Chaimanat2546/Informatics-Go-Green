...
function DashboardContent() {
  ...
  const handleChangeEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessageFn(Array.isArray(data.message) ? data.message.join(', ') : data.message || 'เปลี่ยนอีเมลไม่สำเร็จ', true);
      }
    } catch {
      showMessageFn('เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง', true);
    } finally {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/.test(newPassword)) {
      showMessageFn('รหัสผ่านใหม่ต้องมีตัวพิมพ์เล็ก ตัวพิมพ์ใหญ่ และตัวเลข', true);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessageFn(Array.isArray(data.message) ? data.message.join(', ') : data.message || 'เปลี่ยนรหัสผ่านไม่สำเร็จ', true);
      }
    } catch {
      showMessageFn('เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง', true);
    } finally {
  ...
  const handleDeleteAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessageFn(Array.isArray(data.message) ? data.message.join(', ') : data.message || 'ลบบัญชีไม่สำเร็จ', true);
      }
    } catch {
      showMessageFn('เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง', true);
    } finally {
        ...
        {user && (
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-semibold">สวัสดี, {user.firstName} {user.lastName}!</h2>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="font-medium">{user.provider || 'local'}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <form onSubmit={handleChangeEmail} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">อีเมลใหม่</label>
                <input
                  type="email"
                  value={newEmail}
            ...
            <form onSubmit={handleDeleteAccount} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">
                  {user?.provider === 'local' ? 'กรอกรหัสผ่านเพื่อยืนยัน' : 'พิมพ์ "DELETE" เพื่อยืนยัน'}
                </label>
                <input
                  ...
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full p-2 border rounded"
                  placeholder={user?.provider === 'local' ? '••••••••' : 'DELETE'}
                  required
                />
              </div>
              <div className="flex gap-4">
                ...
                <button
                  type="submit"
                  disabled={actionLoading || (user?.provider !== 'local' && password !== 'DELETE')}
