...
export class WasteSortingService {
    ...
    async findMeterialsAll(page: number = 1, categoryName?: string, materialName?: string) {
        const limit = 6;
        ...
        const [results, total] = await this.wasteMaterialRepository.findAndCount({
