...
export class StatisticsService {
    ...
    private generateEmptyGraph(type: 'daily' | 'monthly' | 'yearly', dateStr: string) {
    ...
    private fillMissingData(emptyGraph: any[], rawDbData: any[]) {
        return emptyGraph.map(emptyItem => {
            const foundDbItem = rawDbData.find(dbItem => {
                const dbLabel = dbItem.time_label ? dbItem.time_label.trim() : '';
                return dbLabel === emptyItem.time_label;
            });

            return {
                time_label: emptyItem.time_label,
                carbon_value: foundDbItem ? parseFloat(foundDbItem.carbon_value) || 0 : 0
            };
        });
    }
    ...
    async getCarbonStats(type: 'daily' | 'monthly' | 'yearly', dateStr: string) {
        ...
        const totalCarbon = parseFloat(summary.totalCarbon) || 0;
        const treeEquivalent = totalCarbon / 9; // หาร 9 เพื่อหาจำนวนต้นไม้ (สมมติ 9kg = 1 ต้น)
    ...
    async getWasteCategoryDistribution(type: 'daily' | 'monthly' | 'yearly', dateStr: string) {
        ...
        return await this.historyRepository
            ...
            .select("COALESCE(categoryFromWaste.name, categoryFromMaterial.name, 'อื่นๆ')", 'categoryName')
            .addSelect('SUM(history.amount)', 'totalWeight')
            .where('history.create_at BETWEEN :start AND :end', { start, end })
            .groupBy("COALESCE(categoryFromWaste.name, categoryFromMaterial.name, 'อื่นๆ')")
            .getRawMany();
    }

    async getOverallSummary(type: 'daily' | 'monthly' | 'yearly', dateStr: string, userId?: string) {
        const { start, end } = getDateRange(type, dateStr);
        ...
        return {
            totalWeight: parseFloat(parseFloat(result?.totalWeight || '0').toFixed(0)),
            totalCarbon: parseFloat(parseFloat(result?.totalCarbon || '0').toFixed(0)),
        };
    }

    async getCardsSummary(type: 'daily' | 'monthly' | 'yearly', dateStr: string, userId?: string) {
        const currentRange = getDateRange(type, dateStr);
        const previousRange = this.getPreviousDateRange(type, currentRange.start, currentRange.end);
        const currentData = await this.fetchSummaryData(currentRange.start, currentRange.end, userId);
        const previousData = await this.fetchSummaryData(previousRange.start, previousRange.end, userId);
        const COST_PER_KG = 8; // ขยะ 1 กิโลกรัม ประหยัดเงินได้ 8 บาท
        const KG_CO2_PER_TREE = 9; // คาร์บอน 9 kgCO2e เทียบเท่าการปลูกต้นไม้ 1 ต้น
        ...
        return [
            {
                ...
                value: currentData.totalWeight.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
                unit: "กิโลกรัม",
                desc: this.calculatePercentageText(currentData.totalWeight, previousData.totalWeight, type),
            },
            {
                id: "carbon",
                title: "ลดการปล่อยก๊าซ CO2 ได้",
                value: currentData.totalCarbon.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
                unit: "kgCO2e",
                desc: this.calculatePercentageText(currentData.totalCarbon, previousData.totalCarbon, type),
            },
            {
                id: "cost",
                title: "ประหยัดค่าใช้จ่ายไปได้",
                value: currentCost.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }),
                unit: "บาท",
    ...
    // Helper: คำนวณเปอร์เซ็นต์การเปลี่ยนแปลง
    private calculatePercentageText(current: number, previous: number, type: string): string {
        const timeText = type === 'daily' ? 'เมื่อวาน' : type === 'monthly' ? 'เดือนที่ผ่านมา' : 'ปีที่ผ่านมา';

        if (previous === 0) {
            return current > 0 ? `เพิ่มขึ้น 100% จาก${timeText}` : `ไม่มีการเปลี่ยนแปลงจาก${timeText}`;
        }
    ...
    private getPreviousDateRange(type: string, currentStart: Date, currentEnd: Date) {
    ...
    async getMonthlyCarbonFootprint(year: number, userId?: string) {
        ...
        query = query.select('EXTRACT(MONTH FROM history.create_at)', 'month')
        ...
        result.forEach(row => {
            ...
            if (monthIndex >= 0 && monthIndex < 12) {
                monthlyData[monthIndex].co2 = parseFloat(parseFloat(row.total_co2 || '0').toFixed(2));
            }
        });
    ...
    async getWasteWeightByCategorySixMonths(userId?: string) {
        ...
        let query = this.historyRepository.createQueryBuilder('history')
            ...
            .addSelect("COALESCE(matCategory.name, wasteCategory.name, 'อื่นๆ')", "categoryName")
            .addSelect("SUM(history.amount)", "totalWeight")
        ...
        rawData.forEach(row => {
            const dataRow = result.find(r => r.year === Number(row.year) && r.monthIndex === Number(row.month));
        ...
        return result.map(({ year, monthIndex, ...rest }) => rest);
    }
